...
const searchService = (tenant: string, search: string, page: number, size: number) => {
    return network.get(`${CUSTOMER_PREFIX}/list/${tenant}/${search}/${page}/${size}`);
}
...
const getAddressService = (id: string) => {
    return network.get(`${CUSTOMER_PREFIX}/${CUSTOMER_ADDRESS_PREFIX}/detail/${id}`);
}

const createAddress = (id: string, data: any) => {
    return network.post(`${CUSTOMER_PREFIX}/${CUSTOMER_ADDRESS_PREFIX}/create/${id}`, data);
}
...
const updateAddress = (id: string, data: any) => {
    return network.post(`${CUSTOMER_PREFIX}/${CUSTOMER_ADDRESS_PREFIX}/update/${id}`, data);
}

const getListAddressService = (id: string, page: number, size: number) => {
    return network.get(`${CUSTOMER_PREFIX}/${CUSTOMER_ADDRESS_PREFIX}/list/${id}/${page}/${size}`);
}
const searchAddressService = (id: string, search: string, page: number, size: number) => {
    return network.get(`${CUSTOMER_PREFIX}/${CUSTOMER_ADDRESS_PREFIX}/list/${id}/${search}/${page}/${size}`);
}
