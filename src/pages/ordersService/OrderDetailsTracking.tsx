...
function OrderDetailsTrackingPage({
  ...
  const showChangeStatusButton = useMemo(() => {
    if (
      orderData.status === ORDER_STATUS_SERVICE.DRIVER_DELIVERED_ITEM_TO_SHOP
    ) {
      return true;
    }
    if (
    ...
    if (
      orderData.status === ORDER_STATUS_SERVICE.PROCESSING_ITEM &&
      orderData.fulfillmentMethod === ORDER_FULFILLMENT_METHOD.SELF
    ) {
      return true;
    }
    if (orderData.status === ORDER_STATUS_SERVICE.CUSTOMER_PICK_UP) {
      return true;
    }
    if (
      orderData.status ===
    ...
    if (
      orderData.status === ORDER_STATUS_SERVICE.DRIVER_RETURNED_ITEM_TO_CUSTOMER
    ) {
      return false;
    }
    if (
    ...
    if (
      orderData.status === ORDER_STATUS_SERVICE.DRIVER_RETURNED_ITEM_TO_SHOP
    ) {
      return false;
    }
    if (orderData.status === ORDER_STATUS_SERVICE.COMPLETED) {
      return false;
    }
    return false;
  ...
  const isStatusActive = useCallback(
    (status: string) => {
      ...
      if (status === ORDER_STATUS_SERVICE.NEW) {
        return true;
      }

      if (status === ORDER_STATUS_SERVICE.DRIVER_ASSIGNED_FOR_ITEM_PICKUP) {
        let pickedUp = true;
        if (
          currentStatus ===
      ...
      if (
        ...
        if (
          currentStatus === ORDER_STATUS_SERVICE.DRIVER_ASSIGNED_FOR_ITEM_PICKUP
        ) {
          showAccepted = false;
        }
        if (
      ...
      if (status === ORDER_STATUS_SERVICE.DRIVER_PICKED_UP_ITEM_FROM_CUSTOMER) {
      ...
      if (status === ORDER_STATUS_SERVICE.DRIVER_RETURNED_ITEM_TO_CUSTOMER) {
      ...
      if (status === ORDER_STATUS_SERVICE.DRIVER_DELIVERED_ITEM_TO_SHOP) {
        return true;
      }

      if (status === ORDER_STATUS_SERVICE.PROCESSING_ITEM) {
        return true;
      }

      if (status === ORDER_STATUS_SERVICE.DRIVER_ASSIGNED_FOR_ITEM_DELIVERY) {
        let pickedUp = true;
        if (
          currentStatus ===
          ORDER_STATUS_SERVICE.DRIVER_DECLINED_TO_PICKUP_ITEM_FROM_SHOP
        ...
        if (
          currentStatus === ORDER_STATUS_SERVICE.DRIVER_RETURNED_ITEM_TO_SHOP
        ) {
          pickedUp = false;
        }
        return pickedUp;
      ...
      if (status === ORDER_STATUS_SERVICE.DRIVER_RETURNED_ITEM_TO_SHOP) {
      ...
      if (status === ORDER_STATUS_SERVICE.DRIVER_PICKED_UP_ITEM_FROM_SHOP) {
        let pickedUp = true;
        if (
          currentStatus ===
          ORDER_STATUS_SERVICE.DRIVER_ASSIGNED_FOR_ITEM_DELIVERY
      ...
      if (status === ORDER_STATUS_SERVICE.DRIVER_DELIVERED_ITEM_TO_CUSTOMER) {
        return true;
      }

      if (status === ORDER_STATUS_SERVICE.CUSTOMER_PICK_UP) {
        return true;
      }

      if (status === ORDER_STATUS_SERVICE.COMPLETED) {
        return true;
      }

      if (status === ORDER_STATUS_SERVICE.CANCELLED) {
        return true;
      }
  ...
  return (
    <div className="mb-auto min-h-[40rem] rounded-lg bg-[#fff] shadow-lg">
      ...
      <div className="flex flex-col gap-4 px-4 py-4">
        {[...ORDER_STATUSES_SERVICE].map(([_key, value], index: number) => {
          const appOrderStatus = orderData.appOrderStatuses?.find(
