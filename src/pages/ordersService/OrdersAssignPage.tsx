...
function OrdersAssignPage() {
  ...
  const newStatus = useMemo(() => {
    ...
    if (data.status === ORDER_STATUS_SERVICE.NEW) {
      return ORDER_STATUS_SERVICE.DRIVER_ASSIGNED_FOR_ITEM_PICKUP;
    }
    if (
      data.status ===
    ...
    if (data.status === ORDER_STATUS_SERVICE.DRIVER_RETURNED_ITEM_TO_CUSTOMER) {
      return ORDER_STATUS_SERVICE.DRIVER_ASSIGNED_FOR_ITEM_PICKUP;
    }
    if (data.fulfillmentMethod === 'Self') {
      return ORDER_STATUS_SERVICE.CUSTOMER_PICK_UP;
    }
    return ORDER_STATUS_SERVICE.DRIVER_ASSIGNED_FOR_ITEM_DELIVERY;
  ...
  const assignButton = (item: any) => {
    ...
    if (item.appOrderDelivery.appOrder === orderId) {
      if (item.appOrderDelivery.status === ORDER_STATUS_SERVICE.NEW)
        return false;
      if (
      ...
      if (
        ...
        return false;
      if (item.appOrderDelivery.status === ORDER_STATUS_SERVICE.PROCESSING_ITEM)
        return false;
      if (
        item.appOrderDelivery.status ===
  ...
  const assignBtnColor = (item: any) => {
    // || item.appOrderDelivery.status === ORDER_STATUS_SERVICE.CANCELLED
    let colorText = '';
    if (item.isActive === false || item.status === APP_USER_STATUS_OFFLINE) {
      colorText = 'btn-gray-fill btn-icon';
    } else if (
      item.appOrderDelivery.status === ORDER_STATUS_SERVICE.NEW ||
