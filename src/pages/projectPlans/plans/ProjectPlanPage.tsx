...
function ProjectPlanPage() {
  ...
  useEffect(() => {
    if (listingRolePermission(dataRole, ALL_PERMISSIONS.storePlans.viewPlans)) {
      // storePlanService
      //   .getListService(authState.user.tenant)
      //   .then((item: any) => {
      //     setIsLoader(false);
  ...
  const createFormHandler = (data: any) => {
    ...
    formData.append('project_id', '55c08a0d-071d-4293-a892-7c976de80563');
    storePlanService
          ...
          <div className="mt-3 grid grid-cols-none">
            <table className="table-border table-auto">
              <thead>
                <tr>
                  <th>Day</th>
                  <th className="w-[10%]">Days Count</th>
              ...
              <tbody>
                {list &&
                  list.map((item: any, index: number) => {
                    const activities = item.activity?.split(', ');
                    return (
                      <tr key={index}>
