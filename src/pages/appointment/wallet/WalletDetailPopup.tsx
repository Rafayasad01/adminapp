...
function WalletDetailPopup({
  ...
  useEffect(() => {
    ...
    setIsLoader(true);
    walletService
      .WalletTransactionList(walletId, page, rowsPerPage, toDate, fromDate)
      .then((item: any) => {
  ...
  const handleChangePage = (
    ...
    setPage(newPage);
    walletService
      .WalletTransactionList(walletId, newPage, rowsPerPage, to, from)
      .then((item) => {
        if (item.data.success) {
          setTotal(item.data.data.total);
  ...
  const handleChangeRowsPerPage = (
    ...
    setPage(newPage);
    walletService
      .WalletTransactionList(walletId, newPage, newRowperPage, to, from)
      .then((item) => {
        if (item.data.success) {
          setTotal(item.data.data.total);
          setList(item.data.data.list);
  ...
  return (
    ...
    >
      <div className="Content">
        ...
        />
        <p className="Title pb-10">Wallet Transactions</p>
        {isLoader ? (
          <Loader />
        ) : (
          <>
            <div className="">
              <LocalizationProvider dateAdapter={AdapterDayjs}>
                <form
                  ...
                  className="mx-2 flex items-end justify-end"
                >
                  <div className="mx-2">
                  ...
                  <div className="mx-2">
                    <Button
                      type="submit"
                      variant="outlined"
                      className="btn-icon col-span-12 items-end md:col-span-3"
                    >
                      Find
                    </Button>
                  </div>
