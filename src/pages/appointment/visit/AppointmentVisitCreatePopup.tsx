...
function AppointmentVisitCreatePopup({
    roles,
    openFormDialog,
    setOpenFormDialog,
    callback,
    ...
    const onSubmit = (data: AppointmentVisit) => {
        ...
        let formattedDate = `${dayjs(data.appointmentDate).isValid() && dayjs(data.appointmentDate)?.format('YYYY-MM-DD')} ${dayjs(startTime).isValid() && dayjs(startTime)?.format('HH:mm:ss')}`
        ...
        let visitorDetails = {
            name: data.visitName,
            phone: data.phone,
            appointmentTime: watch("isCheckTimingSlot") ? formattedDate : formattedCurrentDate,
    ...
    useEffect(() => {
        // setIsLoader(true);
        setServiceLov([])
        if (watch("appointmentProvider") !== "none" && watch("appointmentProvider") !== undefined) {
...
export default AppointmentVisitCreatePopup;
