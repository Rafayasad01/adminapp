...
export default function AddAppointmentPage() {
  ...
  const BarberCard = (item: any, index: number) => {
    ...
    const onHandleBarber = async () => {
      if (index === activeBarber) {
        ...
        if (item.storeEmployee.name !== 'Any Professional') {
          ...
          const anyProfessional = [
            {
              ...
              servicePrice: res.data.data[0].servicePrice,
  ...
  const addAppointmentServices = () => {
    ...
    if (
      ...
      const isDuplicateService = fields.some(
        (el: any) =>
          activeBarberData?.storeEmployee?.id === 'AnyProfessional' &&
          el.storeServiceCategoryItem === watch('storeServiceCategoryItem')
      );

      console.log('isDuplicateService', activeBarberData);
                    ...
                    return (
                      <div className="my-4 grid grid-cols-12" key={index}>
                        ...
                        <div className="col-span-2 mx-5">
                          ...
                          <span className="xl:text-xs 2xl:text-sm">
                            {dayjs(items.appointmentTime).format('YYYY-MM-DD')}
                          </span>
                        </div>
                        <div className="col-span-2 mx-5">
                          <p className="font-semibold xl:text-xs 2xl:text-sm">
