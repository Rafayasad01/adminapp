...
import QuotationSlipAddPopup from './QuotationSlipAddPopup';
import CustomDropDown from '../../../components/common/CustomDropDown';
import { ProjectQuotation } from '../../../interfaces/projectQuotation';
import QuotationSlipEditPopup from './QuotationSlipEditPopup';
// import assets from '../../../assets';

function QuotationSlipPage() {
  ...
  const createFormHandler = (data: any) => {
    // console.log('🚀 ~ createFormHandler ~ data:', data);
    setIsLoader(true);
    const formData = new FormData();
    if (data.file !== null) formData.append('file', data.file);
    formData.append('projectId', data.projectId);
    formData.append('type', data.type);
    formData.append('quotationCost', data.quotationCost);
        ...
        if (item.data.success) {
          ...
          for (let i = 0; i < list.length; i += 1) {
            if (list[i].id === item.data.data.id) {
              ...
              list[i].quotationCost = item.data.data.quotationCost;
                  ...
                  list.map((item: any, index: number) => {
                    return (
                      <tr key={index}>
                        ...
                        <td>
                          {item.quotationCost ? item.quotationCost : '--'}{' '}
                          {CURRENCY_PREFIX}
                        </td>
                        <td>
...
export default QuotationSlipPage;
